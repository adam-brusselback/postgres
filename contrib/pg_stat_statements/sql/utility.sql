...
--
-- Structural assertions on what a partial refresh actually issues.
--
-- Found here, not on the -hackers thread.  With track = 'all',
-- pg_stat_statements sees the statements the refresh runs internally, so the
-- shape of the implementation becomes observable through the real command
-- rather than through a hand-written analogy of it.  That matters because an
-- analogy drifts silently when the implementation changes, which is exactly
-- what a performance pass does.
--
-- These three assertions cover, between them, every structural property the
-- correctness fixes depend on:
--
--   two nested statements   splitting the fused CTE into upsert-then-prune
--                           makes it three, and reopens a consistency gap of
--                           the same family as A3 (see
--                           matview_where_stress/safety/a3-split-gap.sh)
--   ordered locking SELECT  A5: without ORDER BY, two overlapping refreshes
--                           lock in different orders and deadlock
--   fused, materialised CTE A3: one materialised new_data is what stops the
--                           upsert and the prune disagreeing about which rows
--                           exist
--
-- Disposition: DELETE at the start of the Query-tree work, before it begins --
-- not after.  This asserts the mechanism, not the guarantee.  It matches the
-- literal text "new_data AS MATERIALIZED", so it would go red against a correct
-- implementation that delivers the same single-evaluation guarantee some other
-- way: one ModifyTable plan, a tuplestore read twice, anything not spelled that
-- way.  A gate that fails when the work is done properly is worse than no gate.
--
-- What it is reaching for belongs as an Assert() where the guarantee is
-- established, naming A3.  An assertion says "this delivers single evaluation";
-- this test says "this delivers single evaluation BY WRITING A CTE".
--
-- Its only unique coverage is a mutation that removes MATERIALIZED, which is
-- only interesting while the CTE is how the guarantee is delivered.  See
-- src/test/matview_where_stress/PLAN.md.
--
...
SELECT count(*) AS nested_statements
  FROM pg_stat_statements WHERE NOT toplevel;

SELECT query ~ 'ORDER BY .* FOR UPDATE' AS locking_select_is_ordered
  FROM pg_stat_statements
 WHERE NOT toplevel AND query LIKE 'SELECT%FOR UPDATE';

SELECT query ~ 'new_data AS MATERIALIZED' AS new_data_is_materialised,
       query ~ 'upsert AS' AND query ~ 'pruned AS' AS upsert_and_prune_are_fused,
       query ~ 'ORDER BY' AS new_data_is_ordered
  FROM pg_stat_statements
 WHERE NOT toplevel AND query LIKE 'WITH new_data%';
