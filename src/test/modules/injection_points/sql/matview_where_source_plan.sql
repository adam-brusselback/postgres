...
-- therefore cached, and this is the gate on whether that cache does what it
-- claims.
--
...
--
-- The probe is attached only after a warm-up, and that is a deliberate change
-- from the first version of this file.  A partial refresh now replaces the
...
-- "the plan is reused" is an assertion about the steady state, and the way to
-- assert it without also asserting the number 5 in choose_custom_plan() is to
-- let the comparison finish unwatched and then demand silence.
--
...
-- it holds however the plan comes to be stored.
--
