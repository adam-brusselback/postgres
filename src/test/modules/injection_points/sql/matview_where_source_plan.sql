...
-- rebuilt and re-planned the source query on every single call.  That is the
-- 8x of RESULTS.md R15 and most of a scope-1 refresh.  Replacing the
-- predicate's constants with parameters makes those two one statement with
-- different values, and the plan is reused across them.
--
...
-- Against a build that leaves the constants alone -- mutations.py C5 -- all
-- five report "notice triggered for injection point
-- matview-where-source-planned", because each is a different cache key and
