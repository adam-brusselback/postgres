...
| B28 | **The match/merge path is never exercised with a name that needs quoting.** Q2 removes the quoting of the matview name on that path and **no instrument catches it**: the path is reached (`mv_multi2` pins the routing rule with two unique indexes) but only ever with plain lowercase identifiers, so unquoted output is still valid SQL. Q1 and Q3 are both caught, so the direct-modification path is covered and this is the one gap | `matview_where_inject` (**does not cover it**) | **OPEN.** Found by `calibrate-all.sh`: Q2 quiet in all four instruments. The fix is a multi-unique-index matview named so that dropping the quotes is a syntax error or resolves elsewhere -- the same construction Q1/Q3 already use, applied to the other path | add the case |
| B27 | **The row comparison is never exercised against a NULL.** B7 changes it from `IS DISTINCT FROM` to `<>`, which differ only when a value is NULL, and **no instrument catches it**. `matview_where_cache` Test 5 does set `matview_partial_refresh_optimized = on`, so the code runs -- but its data has no NULLs, so the two operators agree on every row. The failure this hides is silent: a NULL-valued row would never be updated | `matview_where_cache` Test 5 (**runs the code, cannot see the bug**) | **OPEN.** Found by `calibrate-all.sh`: B7 quiet in all four instruments. Needs a nullable non-key column and a refresh that should change it to or from NULL | add the case |
| B26 | **The primary-key preference in the arbiter search is dead code.** `refresh_by_direct_modification()` walks the index list preferring `indisprimary`, but is only reached when routing has already established `nUniqueIndexes <= 1` (`indisunique && indisvalid && indimmediate`), and its own filter `is_usable_unique_index()` is stricter still -- it also rejects partial and expression indexes. Usable is therefore a subset of counted, counted is at most one, and the preference can never choose between anything | none, by construction -- mutation `B6` is reclassified `benign` for this reason | **OPEN, cosmetic but worth doing before posting.** Vestigial: it predates routing moving to the unique-index count (B21) and was not updated with it. A reviewer reading it will reasonably assume the function can see several unique indexes, which it cannot. Deleting the `is_pk`/`break` branch leaves "take the one usable index" | delete the preference |
