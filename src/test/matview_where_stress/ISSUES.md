...
| B32 | **`matview-where-snapshot` is flaky, the same way B20 was.**  Two steps that are both waiting complete in whichever order the scheduler picks, and the expected file records one of them: `step s1_ref: <... completed>` before `step s2b_ref` in the file, the other way round in a failing run.  Observed **once in four consecutive runs** of an otherwise untouched tree -- green on the three after it -- while verifying the text-path deletion, which does not touch this spec.  Not caused by that change; found by it | `injection_points` `matview-where-snapshot` | **OPEN.**  B20's fix is the mechanism: an alternative expected file, which is what the tester provides for legitimately variable output.  Not applied here because the alternative ordering has to be *captured from a failing run* rather than hand-written -- an expected file typed out by hand is a test that cannot fail, which is the defect this directory keeps rediscovering.  Re-run until it fails, then take the output | keep, with the alternative file |
