...
BEGIN
  ...
  -- A predicate requires CONCURRENTLY, so there is no second form to sweep.
  -- Raise rather than fall back to '': an unknown form used to build a bare
  -- REFRESH ... WHERE, which the grammar now rejects, and a silently invalid
  -- statement is the one failure this harness must not produce itself.
  IF p_form <> 'concurrently' THEN
    RAISE EXCEPTION 'unknown refresh form: %', p_form;
  END IF;
  conc := 'CONCURRENTLY ';
