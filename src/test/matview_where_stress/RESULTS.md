...
| R30 | the concurrent fuzzer's `nest` mode | **coverage, not a detector — the calibration is NEGATIVE.** Against a build with `mutations.py` C2 applied (B14's guard removed) it fired **once**, then **0 in 6** at 4×25 and **0 in 3** at 8×60 on the same binary. The single fire is unexplained and is not a rate. Concurrency makes this class *harder* to hit, not easier: the single-session test controls the ordering and lands it every time, this one needs a window microseconds wide | `fuzz.sh nest`, both GUC arms alternated across sessions, with an ALTER TABLE invalidator committing in a separate backend | `fuzz.sh`; ISSUES.md B14 | settled |
