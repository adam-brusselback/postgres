...
| R31 | do `p2`/`p3` notice a refresh that never ran? | **they did not.** With A4 applied — `ON CONFLICT` dropped, so every refresh dies of a unique violation — **40 of 40 failed**, the transactions rolled back, the matview was therefore still correct, and the run reported "no deadlocks", "matview matches its definition" and **PASS**.  Both modes now count non-deadlock errors and fail on them | A4 applied, `fuzz.sh p2 p3`, 4 sessions × 10 | `fuzz.sh` | settled || R20 | mutation corpus | **17/17 apply** against the current tree (C2 added: reinstates B14's use-after-free) | `mutations.py --check`; an edit declares its expected occurrence count | ISSUES.md B23 | settled |
