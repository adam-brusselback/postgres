...
| R31 | do `p2`/`p3` notice a refresh that never ran? | **they did not.** With A4 applied — `ON CONFLICT` dropped, so every refresh dies of a unique violation — **40 of 40 failed**, the transactions rolled back, the matview was therefore still correct, and the run reported "no deadlocks", "matview matches its definition" and **PASS**.  Both modes now count non-deadlock errors and fail on them | A4 applied, `fuzz.sh p2 p3`, 4 sessions × 10 | `fuzz.sh` | settled || R31 | **every mutation against every instrument, both directions** | **12 CAUGHT, 3 benign-and-quiet, 2 UNCAUGHT, pristine QUIET.** Caught: A4 B4 C1 C2 M1 M2 M3 M6 P1 Q1 Q3 S1. Correctly quiet: B4b M4 **B6** (reclassified benign -- its preference is dead code, ISSUES.md B26). **Uncaught at the time: B7** (row comparison never met a NULL, B27) and **Q2** (match/merge path never met a name needing quotes, B28) -- **both now covered and both seen to fail**: `matview_where` Test 17 and `matview_where_inject` Test 9, so the corpus is **14 caught, 2 benign-and-quiet, 0 uncaught** over 16 mutations — B6 was retired with the dead code it targeted (ISSUES.md B26), and what its search still does that is untested is now ISSUES.md B29.  Six mutations -- C1 C2 P1 Q1 Q2 Q3 -- had never been verified by any harness before this run | `calibrate-all.sh`, one build per mutation, four instruments each: regression suite, differential oracle, concurrent fuzzer, isolation + injection-point specs | `calibrate-all.sh`; ISSUES.md B26-B28 | settled |
| R20 | mutation corpus | **16/16 apply** against the current tree.  C2 added (reinstates B14's use-after-free); **B6 deleted** — its target, the primary-key preference, was dead code and is gone (ISSUES.md B26) | `mutations.py --check`; an edit declares its expected occurrence count | ISSUES.md B23 | settled |
