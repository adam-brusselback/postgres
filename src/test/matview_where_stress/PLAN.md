...
| 3.7 cache the source plan | **pursue, required — NEXT.  Subplan: CACHE.md** | 12.20 µs, **16.2%** of a warm Query-tree refresh (R13), and the entire reason it currently loses.  Recoverable in full in the constant-literal cell R26 was measured in, where `params` is NULL and the plan is generic unconditionally |
...
- Every test that only existed to guard the old mechanism is deleted, not
  ...
  | `matview_where` Tests 17 and 18, added for B27 and B30 | the optimisation stops being optional | **read at Phase 4** — the tests stay, only their `SET` lines go. Both properties outlive the flag: a comparison that is not NULL-safe corrupts the matview, and not rewriting a row nothing changed about is the promise that makes re-refreshing an already-current scope cheap |
