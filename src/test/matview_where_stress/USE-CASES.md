...
- Both forms are **linear** in scope — 10.1 / 11.6 / 15.2 µs/row for
  ...
  is now measured rather than taken from the thread** (RESULTS.md **R51**): with
  a moving window, v2's bare form runs 0.47 → 0.62 → 1.88 → 83.9 → **7558 ms**
  across scope 1 to 10,000 where the current one runs 0.38 → 0.45 → 0.47 → 2.42
  → **21.7**, which is **347× at the top**.  Until R51 this claim rested on the
  current code measuring linear plus an assertion about v2 that nothing in this
  tree had run.
