...
| **index shape** | do the updated columns sit under any index? | whether an avoided write saves index maintenance or just a HOT update | measured: the row comparison is **27–53% faster** with a covering index and ~~makes no difference at all without one~~ — **that half is retracted (R45, X17): with no index over a written column it still reads +38.5% at scope 1000.**  The avoided update is only HOT while the page has free space, and a matview refreshed repeatedly without a vacuum between refreshes does not have it; most of the saving is not writing a row version at all.  **There is no static veto here.** Treat the magnitude as an upper bound — it was measured fresh-heap (see below) — but not the presence/absence result, which compares two arms that both write |
| **churn fraction** | share of the scope whose values actually changed | whether the row comparison pays | measured, `bench/churn.sql`, on `nonkey`/scope 10000 — **faster is better, and it runs out**: **54–56% faster** at churn 0, then 41–51 → 27–36 → 18–27 → **≈0%** at churn 5/25/50/100 (ranges are two protocols disagreeing). At full churn every comparison fails and the row-wise `IS DISTINCT FROM` is bought for nothing. ~~Break-even ≈ 60–70% churn~~ — **the curve reproduces on current code and the break-even does not (R46, X18)**: +53.8% at churn 0, +36.8% at 25, **+16.8% at 50**, and a wash at 75 and 100 where the three statistics disagree. A curve reading 18–27% faster at churn 50 never had its break-even at 60–70%. **No churn regime measurably loses**, and R47 says why rather than merely that: the comparison costs **24 ns a row** and the write it may avoid costs **~3.6 µs a row**, about 1:150, so it pays as long as more than ~0.7% of the scope is unchanged.  In the phase profile it is the whole of `dmlexec` at churn 0 and invisible at churn 100 |
...
| **parameterise predicate `Const`s** — *implemented*; each Const becomes a `Param` of the same type, typmod and collation before the deparse, and the values ride the `ParamListInfo` the path already carried | — | **8× in-backend** (R15), **0.3-45.3% end to end across 40 paired cells, none slower** (R37).  It turns refreshes that would miss the plan cache into hits.  **One case it can lose, and R41 is narrower than it reads**: a predicate whose *skewed* column drives the plan choice.  R41 tested a single column and found no penalty; a conjunction like `tenant_id = $1 AND status = $2` over a tenant holding 90% of the rows has `auto` settle on generic and costs the heavy tenant ~20-25% (**R50**, provisional).  A conjunction carrying an equality on the arbiter key — `tenant_id = $1 AND sale_id = $2` — is safe by construction, since custom and generic are then the same plan.  Before parameterisation each literal was its own cache key and got its own plan, so this is a real trade and not a free win; recovery is `plan_cache_mode = force_custom_plan` | oracle, plus `matview_where` Test 19 for the values reaching the right rows and `matview_where_source_plan` part 3 for the plan actually being reused.  Changes what the cache key *is*, so it lands first or last, never in the middle |
...
- ~~T3 churn gate next — turns the row comparison from a GUC into a decision.~~
  **The GUC part is done and the gate turned out not to be needed.** The
  ...
  the item is nothing at all: swept across churn 0/5/25/50/75/100 on current
  code, the comparison is +53.8% / +53.8% / +36.8% / **+16.8%** / +4.9% / +3.2%,
  and the two cells above 50% have their three statistics disagreeing, which
  puts them at the floor. **No churn regime measurably loses** (R46), so there
  is no loss for a churn gate to recover and the item is closed rather than
  deferred. Re-open it only with a workload that shows the loss, which nothing
  here does.
...
- ~~The pre-lock `ORDER BY` elision whenever the bare form is being worked on
  anyway.~~ **Done, and rejected on its own evidence — R44.** The condition
  fired (`no_delete` had already put a lock-level test in the right place), the
  change was three lines, it was sound, and it is a net loss: 137 µs where it
  helps against 613 µs where plancache reacts to the cheaper statement by
  re-planning it every call. Recorded here rather than deleted because the
  reasoning that put it on this list was good and the conclusion still went the
  other way — R10's 20–69% is a real sub-component number and was never a
  whole-refresh saving you could add up (X16). Do **not** re-propose it without
  a cell where it is measured end to end.
...
per statement, not the ~20 a noisy run suggested), and **dropping the pre-lock's `ORDER BY` under `ExclusiveLock`**
(R44: sound, and a net loss).
