...
--
-- Tests for the hash opclass on record_image_ops
--
-- The hash functions must agree with the *= operator: whenever a *= b, both
-- hash_record_image() and hash_record_image_extended() must return equal
-- values.  The converse is not required.  Actual hash values are
-- machine-dependent, so we only ever check agreement, never print them.
--

create function image_hash_agrees(a record, b record) returns bool
language plpgsql immutable as $$
begin
    -- unequal inputs place no requirement on the hashes
    if not (a operator(pg_catalog.*=) b) then
        return true;
    end if;
    return hash_record_image(a) = hash_record_image(b)
       and hash_record_image_extended(a, 0) = hash_record_image_extended(b, 0)
       and hash_record_image_extended(a, 42) = hash_record_image_extended(b, 42);
end $$;

-- A corpus covering by-value types, by-reference fixed-length types (which
-- carry internal padding), varlena types, and NULLs.  Several rows are
-- image-distinct but semantically equal (numeric 1.0 vs 1.00, float8 0.0 vs
-- -0.0), and several are duplicates reached by different expressions.
create type ri_corpus_t as (
    i2 smallint, i4 int, i8 bigint, b bool, f4 float4, f8 float8,
    nm name, iv interval, uu uuid,          -- by-ref, fixed length
    t text, by bytea, nu numeric            -- varlena
);

create temp table ri_corpus (id int, r ri_corpus_t);
insert into ri_corpus values
 (1,  row(null,null,null,null,null,null,null,null,null,null,null,null)),
 (2,  row(0,0,0,false,0,0,'','0','00000000-0000-0000-0000-000000000000','','','0')),
 (3,  row(0,0,0,false,0,0,'','0','00000000-0000-0000-0000-000000000000','','','0')),
 (4,  row(1,1,1,true,1,1,'a','1 day','11111111-1111-1111-1111-111111111111','a','\x00','1')),
 (5,  row(1,1,1,true,1,1,'a','1 day','11111111-1111-1111-1111-111111111111','a','\x00','1')),
 (6,  row(-1,-1,-1,true,-1,-1,'b','-1 day','22222222-2222-2222-2222-222222222222','b','\x01','-1')),
 -- numeric 1.0 vs 1.00: image-distinct, semantically equal
 (7,  row(1,1,1,true,1,1,'a','1 day','11111111-1111-1111-1111-111111111111','a','\x00','1.0')),
 (8,  row(1,1,1,true,1,1,'a','1 day','11111111-1111-1111-1111-111111111111','a','\x00','1.00')),
 -- float8 0.0 vs -0.0: image-distinct, semantically equal
 (9,  row(0,0,0,false,0,'-0.0','','0','00000000-0000-0000-0000-000000000000','','','0')),
 -- NaN
 (10, row(0,0,0,false,'NaN','NaN','','0','00000000-0000-0000-0000-000000000000','','','NaN')),
 (11, row(0,0,0,false,'NaN','NaN','','0','00000000-0000-0000-0000-000000000000','','','NaN')),
 -- a single NULL field among non-NULLs
 (12, row(1,1,1,true,1,1,'a','1 day','11111111-1111-1111-1111-111111111111','a','\x00',null));

-- must return no rows
select a.id, b.id from ri_corpus a, ri_corpus b
 where not image_hash_agrees(a.r, b.r);

-- sanity check that the corpus really does contain *=-equal pairs, so that
-- the test above is not vacuous
select count(*) as image_equal_pairs from ri_corpus a, ri_corpus b
 where a.r::record operator(pg_catalog.*=) b.r::record;

-- domains over the above types must behave identically
create domain ri_dom_num as numeric;
create domain ri_dom_txt as text;
create type ri_dom_t as (a ri_dom_num, b ri_dom_txt);
select row(1.0::ri_dom_num, 'x'::ri_dom_txt)::ri_dom_t::record
       operator(pg_catalog.*=) row(1.00::ri_dom_num, 'x'::ri_dom_txt)::ri_dom_t::record
       as dom_numeric_image_eq;
select image_hash_agrees(row(1.0::ri_dom_num, 'x'::ri_dom_txt)::ri_dom_t,
                         row(1.0::ri_dom_num, 'x'::ri_dom_txt)::ri_dom_t) as dom_agrees;

-- The same logical value stored inline, out-of-line uncompressed, and
-- compressed must be *=-equal, so the hashes must match too.
create temp table ri_toast (id int, a text);
alter table ri_toast alter column a set storage external;   -- out-of-line, no compression
insert into ri_toast values (1, repeat('abcde', 10000));
create temp table ri_comp (id int, a text);
alter table ri_comp alter column a set storage extended;    -- compressed
insert into ri_comp values (1, repeat('abcde', 10000));

select (select row(a)::record from ri_toast) operator(pg_catalog.*=)
       (select row(a)::record from ri_comp) as toast_vs_compressed_image_eq;
select image_hash_agrees((select row(a) from ri_toast),
                         (select row(a) from ri_comp)) as toast_vs_compressed_agrees;
select image_hash_agrees((select row(a) from ri_toast),
                         row(repeat('abcde', 10000))) as toast_vs_inline_agrees;
select image_hash_agrees((select row(a) from ri_comp),
                         row(repeat('abcde', 10000))) as compressed_vs_inline_agrees;

-- Dropped columns: record_image_eq skips them independently on each side, so
-- two rows with differently-shaped tuple descriptors can be equal.  The hash
-- must skip them the same way.
create temp table ri_dropped (a int, junk text, b text);
alter table ri_dropped drop column junk;
insert into ri_dropped values (1, 'x');
create temp table ri_nodropped (a int, b text);
insert into ri_nodropped values (1, 'x');

select d.*::record operator(pg_catalog.*=) n.*::record as dropped_image_eq
  from ri_dropped d, ri_nodropped n;
select image_hash_agrees(d.*, n.*) as dropped_agrees
  from ri_dropped d, ri_nodropped n;

-- Unlike hash_record, image hashing needs no per-column hash function, so it
-- works for column types that have no hash opclass at all.
select hash_record(row(point '(1,2)', 3));                      -- fails
select hash_record_image(row(point '(1,2)', 3)) is not null as image_hash_works;
select image_hash_agrees(row(point '(1,2)', 3), row(point '(1,2)', 3)) as unhashable_col_agrees;

-- the opclass must be usable for a hash index
create temp table ri_idx (a int, b text);
insert into ri_idx select g, 'v' || g from generate_series(1, 20) g;
create index ri_idx_hash on ri_idx using hash ((ri_idx.*) record_image_ops);
drop index ri_idx_hash;

drop table ri_corpus, ri_toast, ri_comp, ri_dropped, ri_nodropped, ri_idx;
drop type ri_corpus_t, ri_dom_t;
drop domain ri_dom_num, ri_dom_txt;
drop function image_hash_agrees(record, record);
