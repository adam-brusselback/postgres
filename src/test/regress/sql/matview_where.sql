...
-- and Test 7 will need its expected output updated at the same time.
--
...
-- Direct modification absorbs the collision.
...
--
-- Disposition: keep the subquery half; REVISIT the search_path half.  If the
-- privilege fix moves predicate analysis to the caller's search_path, the
-- unqualified reference starts working and this becomes an assertion of the new
-- behaviour rather than a record of the old.  If the restricted path stays, it
-- should move into the documentation instead of being asserted here.
--
...
-- match/merge.  That swap is not in the tree.
--
...
--
-- Disposition: DELETE once the swap lands.  This asserts a lock level rather
-- than a behaviour, which is exactly the kind of test that breaks on unrelated
-- locking changes.  It is here to pin the agreed direction until it is
-- implemented, not to live in the tree afterwards.  What should survive is the
-- documented statement of which form blocks writers.
--
...
--
-- Disposition: keep.  That a refresh accepts every row set a full refresh
-- accepts is an invariant, and it is easy to reintroduce by changing how the
-- arbiter index is chosen.
--
