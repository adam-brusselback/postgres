...
--
-- What a partial refresh does: which rows the condition selects, which it
-- leaves alone, and what the command refuses.
--
...
--
-- The pair used to be the two forms, before a WHERE clause required
...
--
-- It pins the precondition in both directions, the refusal and the same
-- command succeeding once the extra index is gone, and
...
--
-- Nothing else exercises an index with INCLUDE columns on this path.
--
...
--
-- "A refresh that changes nothing changes nothing" is worth asserting whichever
-- way the disagreement is resolved, either by matching the index's NULL
-- semantics in the anti-join or by requiring NOT NULL or NULLS NOT DISTINCT key
-- columns.
--
...
-- arbiter index, so the upsert and the anti-join agree that the
...
-- Test 12: Scope drift
--
-- The refresh resolves this with an INSERT ... ON CONFLICT DO UPDATE against
-- the arbiter index, or DO NOTHING when there are no non-key columns.
--
-- A row drifting into the predicate's scope while a stale copy of it sits
-- outside is what the arbiter index exists to reconcile.  Getting it wrong is a
-- duplicate key violation rather than a wrong answer, so it fails loudly.
--
...
--
-- The subquery half was reported by Dharin Shah, who noted that
...
--
-- Both halves matter.  The restricted search path stays, because the predicate
-- runs as the matview owner and a caller-supplied search_path plus
-- owner-context execution is the CVE-2018-1058 shape.  So the
...
-- search_path, which is what the expected output below pins.
--
...
-- no NULLs, and the two operators agree on every non-NULL row, so replacing
-- IS DISTINCT FROM with <> passed the entire suite.
--
...
-- right either way, so nothing that reads values back can see the difference.
-- Reading the ctid is what makes the case a detector.
--
...
--
-- Not rewriting a row nothing changed about is the feature's promise about
-- write amplification.  It is what makes re-refreshing an already-current
-- scope cheap, and the reason a drain can be run often.
--
...
-- with different values, worth about 250 us a refresh, which is most of a
-- scope-1 refresh's cost.
--
...
--
-- "A refresh acts on the rows its predicate names" is the feature's first
-- promise and does not depend on how the predicate is stored.
--
...
-- gets one assigned at parse time, and the two must agree.  The same seam from
-- the other side is an unassigned collation, which made "tag = 'hot'" fail
-- outright.  Two refreshes again, sharing one deparsed predicate and
...
--
-- These are not tests of an optimisation.  They test the promise an
-- optimisation is allowed to keep, that a row leaving the scope is deleted, in
-- the cases where a cheaper implementation would stop keeping it.  They outlive
...
-- the elision exists for, where every source row matches a row already in
-- scope, and the second is the same statement when a row really has gone.
...
-- selects, so the DELETE cannot match, since the upsert's own insert is not
...
-- refresh.
--
-- Correlated predicates reach parts of the generated SQL that no other case
-- does, and the failure is loud rather than silent.
--
...
--
-- Test 18 covers a few forms in detail.  This covers the space: every way a
...
-- form.  The predicate names a set of rows; evaluate it directly to learn
...
-- at all, with the same value and the same ctid.  A predicate that silently widened would
...
--
-- The aliasing rows are the only place anything checks that the generated
-- SQL's own names survive a predicate that uses them.
--
...
--
-- Found by running the cases from the thread "Two issues with REFRESH
-- MATERIALIZED VIEW CONCURRENTLY", which is about the duplicate
