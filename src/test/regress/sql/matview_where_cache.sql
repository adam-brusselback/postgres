...
--
-- refresh_by_direct_modification() caches the SPI plans it builds in a
-- session-level hash table keyed on the matview OID.  An entry is reused when
-- the arbiter index OID, the predicate and the parameter types all match, and
-- when the plans are still valid.
--
-- Every case here depends on cache state accumulated over the session, so they
-- live in their own file and must run in the order written.
--
-- Each case covered a live defect when it was written, and the expected output
-- has always described what the command should do, so each one failed until its
-- defect was fixed.  Where the pre-fix behaviour says what the case is for, the
-- comment on the case gives it.
--
...
-- itself cannot succeed, because rows in a second matview cannot be locked
-- while the first is being maintained, but it fails at the locking step well
-- after the
...
-- agrees with the code: SetMatViewPopulatedState() early-returns when the state
...
-- that text spells the old name.  plancache invalidates them, since it
-- registers a syscache callback on pg_proc which this cache does not, and
-- re-analyses
...
--
-- Found by asking whether a rename could still reach a cached plan once the
-- view definition stopped being text.  It can, by a route the original hazard
-- note did not describe, and
...
-- produced it and the predicate never named it, which is silent data loss.
-- With no replacement, the refresh raised "function public.mv_c7_pred(integer) does not
